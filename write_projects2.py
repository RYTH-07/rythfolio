...
export default function Projects() {
  const projects = [
    {
      title: "Rythfolio",
      description: "Personal developer portfolio built with React, Vite, and Tailwind CSS v4. Designed in Figma Make and configured from scratch — involved setting up Vite from a non-standard export, fixing missing entry points, writing all component files via Python scripts due to PowerShell JSX limitations, integrating live GitHub contribution heatmap via public API, dynamic LeetCode stats, particle canvas animation with error boundary, dark/light mode toggle, and deploying on Vercel with auto-deploy on push. Tech learned: React, TypeScript, Tailwind CSS v4, Vite config, Git workflow, Vercel deployment.",
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
          {projects.map((project) => (
            <div key={project.title} className="rounded-xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-950 p-6 flex flex-col hover:shadow-sm transition-shadow">
              ...
              <div className="flex flex-wrap gap-2 mb-4">
                {project.tags.map((tag) => (
                  <span key={tag} className="text-xs px-2.5 py-0.5 rounded-full bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-400">{tag}</span>
                ))}
              </div>
